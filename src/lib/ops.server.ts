...
export function opsAllowlist(): Set<string> {
  const listed = new Set(parseAllowlist(process.env["OPS_ALLOWLIST"]));
...
function asOpsAdmin(admin?: OpsAdminLike): OpsAdminLike {
  return admin ?? (supabaseAdmin as unknown as OpsAdminLike);
}
...
export async function loadOpsSnapshotHandler(claims: { email?: unknown }): Promise<OpsSnapshot> {
  ...
  try {
    const [usersRes, householdsRes] = await Promise.all([
      ...
      supabaseAdmin.from("households").select("id, name, owner_user_id, created_at"),
    ]);
    ...
    await collapseDuplicateIdentitiesToHouseholdOwner(users, households);
...
export async function resendOpsConfirmHandler(
  ...
  try {
    const client = asOpsAdmin(admin);
    const got = await client.auth.admin.getUserById(userId);
    ...
    const sent = await client.auth.resend({ type: "signup", email: user.email, options: { emailRedirectTo: AUTH_REDIRECT } });
    if (sent.error) return { ok: false, error: "Could not send" };

    const after = await client.auth.admin.getUserById(userId);
