...
/** Map keys coerce via Number(new Map()) (NaN) and would steal recency. JSON/PostgREST never sends Map week keys. leftover missing-key drafts stay selectable. */
function leftoverMapKey(value: unknown): boolean {
  try {
...
/** Set keys coerce via Number(new Set()) (NaN) and would steal recency. JSON/PostgREST never sends Set week keys. leftover missing-key drafts stay selectable. */
function leftoverSetKey(value: unknown): boolean {
  try {
...
/** WeakMap keys coerce via Number(new WeakMap()) (NaN) and would steal recency. JSON/PostgREST never sends WeakMap week keys. leftover missing-key drafts stay selectable. */
function leftoverWeakMapKey(value: unknown): boolean {
  try {
...
/** WeakSet keys coerce via Number(new WeakSet()) (NaN) and would steal recency. JSON/PostgREST never sends WeakSet week keys. leftover missing-key drafts stay selectable. */
function leftoverWeakSetKey(value: unknown): boolean {
  try {
...
/** Promise keys coerce via Number(Promise.resolve()) (NaN) and would steal recency. JSON/PostgREST never sends Promise week keys. leftover missing-key drafts stay selectable. */
function leftoverPromiseKey(value: unknown): boolean {
  try {
...
/** Error keys coerce via Number(new Error()) (NaN) and would steal recency. JSON/PostgREST never sends Error week keys. leftover missing-key drafts stay selectable. */
function leftoverErrorKey(value: unknown): boolean {
  try {
...
/** RegExp keys coerce via Number(/2026/) (NaN) and would steal recency. JSON/PostgREST never sends RegExp week keys. leftover missing-key drafts stay selectable. */
function leftoverRegExpKey(value: unknown): boolean {
  try {
...
/** DataView keys coerce via Number(new DataView(new ArrayBuffer(8))) (NaN) and would steal recency. JSON/PostgREST never sends DataView week keys. leftover missing-key drafts stay selectable. */
function leftoverDataViewKey(value: unknown): boolean {
  try {
...
/** TypedArray keys coerce via Number(new Uint32Array([2026])) === 2026 and Number(new Uint8Array([2])) === 2 and would steal recency. JSON/PostgREST never sends TypedArray week keys. leftover missing-key drafts stay selectable. */
function leftoverTypedArrayKey(value: unknown): boolean {
  try {
...
/** Blob keys coerce via Number(new Blob()) (NaN) and Number(new Blob(["2026"])) (NaN) and would steal recency. JSON/PostgREST never sends Blob week keys. leftover missing-key drafts stay selectable. */
function leftoverBlobKey(value: unknown): boolean {
  try {
...
/** File keys coerce via Number(new File(["2026"], "year")) (NaN) and Number(new File([], "empty")) (NaN) and would steal recency. JSON/PostgREST never sends File week keys. leftover missing-key drafts stay selectable. */
function leftoverFileKey(value: unknown): boolean {
  try {
...
/** URL keys coerce via Number(new URL("https://example.com/2026")) (NaN) and Number(new URL("https://example.com/")) (NaN) and would steal recency. JSON/PostgREST never sends URL week keys. leftover missing-key drafts stay selectable. */
function leftoverUrlKey(value: unknown): boolean {
  try {
...
/** FormData keys coerce via Number(new FormData()) (NaN) and Number(new FormData()) after append("year","2026") (NaN) and would steal recency. JSON/PostgREST never sends FormData week keys. leftover missing-key drafts stay selectable. */
function leftoverFormDataKey(value: unknown): boolean {
  try {
...
/** Headers keys coerce via Number(new Headers()) (NaN) and Number(new Headers()) after append("year","2026") (NaN) and would steal recency. JSON/PostgREST never sends Headers week keys. leftover missing-key drafts stay selectable. */
function leftoverHeadersKey(value: unknown): boolean {
  try {
...
/** Request keys coerce via Number(new Request("https://example.com/2026")) (NaN) and Number(new Request("https://example.com/")) (NaN) and would steal recency. JSON/PostgREST never sends Request week keys. leftover missing-key drafts stay selectable. */
function leftoverRequestKey(value: unknown): boolean {
  try {
...
/** Response keys coerce via Number(new Response("2026")) (NaN) and Number(new Response()) (NaN) and would steal recency. JSON/PostgREST never sends Response week keys. leftover missing-key drafts stay selectable. */
function leftoverResponseKey(value: unknown): boolean {
  try {
...
/** ReadableStream keys coerce via Number(new Blob(["2026"]).stream()) (NaN) and Number(new ReadableStream()) (NaN) and would steal recency. JSON/PostgREST never sends ReadableStream week keys. leftover missing-key drafts stay selectable. */
function leftoverReadableStreamKey(value: unknown): boolean {
  try {
...
/** TransformStream keys coerce via Number(new TransformStream({ transform(chunk, controller) { controller.enqueue(chunk); } })) (NaN) and Number(new TransformStream()) (NaN) and would steal recency. JSON/PostgREST never sends TransformStream week keys. leftover missing-key drafts stay selectable. */
function leftoverTransformStreamKey(value: unknown): boolean {
  try {
...
/** CompressionStream keys coerce via Number(new CompressionStream("gzip")) (NaN) and Number(new CompressionStream("deflate")) (NaN) and would steal recency. JSON/PostgREST never sends CompressionStream week keys. leftover missing-key drafts stay selectable. */
function leftoverCompressionStreamKey(value: unknown): boolean {
  try {
    return typeof CompressionStream === "function" && value instanceof CompressionStream;
  } catch {
    return true;
  }
}
...
/** TextEncoderStream keys coerce via Number(new TextEncoderStream()) (NaN) and Number(new TextEncoderStream()) (NaN) and would steal recency. JSON/PostgREST never sends TextEncoderStream week keys. leftover missing-key drafts stay selectable. */
function leftoverTextEncoderStreamKey(value: unknown): boolean {
  try {
    return typeof TextEncoderStream === "function" && value instanceof TextEncoderStream;
  } catch {
    return true;
  }
}
...
export function recency(
  ...
  if (leftoverMapKey(aYearKey)) a.season_year = 0;
  if (leftoverMapKey(aNumKey)) a.week_number = 0;
  if (leftoverMapKey(bYearKey)) b.season_year = 0;
  if (leftoverMapKey(bNumKey)) b.week_number = 0;
  // leftover set keys cannot hide a newer week
  if (leftoverSetKey(aYearKey)) a.season_year = 0;
  if (leftoverSetKey(aNumKey)) a.week_number = 0;
  if (leftoverSetKey(bYearKey)) b.season_year = 0;
  if (leftoverSetKey(bNumKey)) b.week_number = 0;
  // leftover weakmap keys cannot hide a newer week
  if (leftoverWeakMapKey(aYearKey)) a.season_year = 0;
  if (leftoverWeakMapKey(aNumKey)) a.week_number = 0;
  if (leftoverWeakMapKey(bYearKey)) b.season_year = 0;
  if (leftoverWeakMapKey(bNumKey)) b.week_number = 0;
  // leftover weakset keys cannot hide a newer week
  if (leftoverWeakSetKey(aYearKey)) a.season_year = 0;
  if (leftoverWeakSetKey(aNumKey)) a.week_number = 0;
  if (leftoverWeakSetKey(bYearKey)) b.season_year = 0;
  if (leftoverWeakSetKey(bNumKey)) b.week_number = 0;
  // leftover promise keys cannot hide a newer week
  if (leftoverPromiseKey(aYearKey)) a.season_year = 0;
  if (leftoverPromiseKey(aNumKey)) a.week_number = 0;
  if (leftoverPromiseKey(bYearKey)) b.season_year = 0;
  if (leftoverPromiseKey(bNumKey)) b.week_number = 0;
  // leftover error keys cannot hide a newer week
  if (leftoverErrorKey(aYearKey)) a.season_year = 0;
  if (leftoverErrorKey(aNumKey)) a.week_number = 0;
  if (leftoverErrorKey(bYearKey)) b.season_year = 0;
  if (leftoverErrorKey(bNumKey)) b.week_number = 0;
  // leftover regexp keys cannot hide a newer week
  if (leftoverRegExpKey(aYearKey)) a.season_year = 0;
  ...
  if (leftoverDataViewKey(aYearKey)) a.season_year = 0;
  if (leftoverDataViewKey(aNumKey)) a.week_number = 0;
  if (leftoverDataViewKey(bYearKey)) b.season_year = 0;
  if (leftoverDataViewKey(bNumKey)) b.week_number = 0;
  // leftover typedarray keys cannot hide a newer week
  if (leftoverTypedArrayKey(aYearKey)) a.season_year = 0;
  if (leftoverTypedArrayKey(aNumKey)) a.week_number = 0;
  if (leftoverTypedArrayKey(bYearKey)) b.season_year = 0;
  if (leftoverTypedArrayKey(bNumKey)) b.week_number = 0;
  // leftover blob keys cannot hide a newer week
  if (leftoverBlobKey(aYearKey)) a.season_year = 0;
  if (leftoverBlobKey(aNumKey)) a.week_number = 0;
  if (leftoverBlobKey(bYearKey)) b.season_year = 0;
  if (leftoverBlobKey(bNumKey)) b.week_number = 0;
  // leftover file keys cannot hide a newer week
  if (leftoverFileKey(aYearKey)) a.season_year = 0;
  if (leftoverFileKey(aNumKey)) a.week_number = 0;
  if (leftoverFileKey(bYearKey)) b.season_year = 0;
  if (leftoverFileKey(bNumKey)) b.week_number = 0;
  // leftover url keys cannot hide a newer week
  if (leftoverUrlKey(aYearKey)) a.season_year = 0;
  ...
  // leftover formdata keys cannot hide a newer week
  if (leftoverFormDataKey(aYearKey)) a.season_year = 0;
  if (leftoverFormDataKey(aNumKey)) a.week_number = 0;
  if (leftoverFormDataKey(bYearKey)) b.season_year = 0;
  if (leftoverFormDataKey(bNumKey)) b.week_number = 0;
  // leftover headers keys cannot hide a newer week
  if (leftoverHeadersKey(aYearKey)) a.season_year = 0;
  if (leftoverHeadersKey(aNumKey)) a.week_number = 0;
  if (leftoverHeadersKey(bYearKey)) b.season_year = 0;
  if (leftoverHeadersKey(bNumKey)) b.week_number = 0;
  // leftover request keys cannot hide a newer week
  if (leftoverRequestKey(aYearKey)) a.season_year = 0;
  if (leftoverRequestKey(aNumKey)) a.week_number = 0;
  if (leftoverRequestKey(bYearKey)) b.season_year = 0;
  if (leftoverRequestKey(bNumKey)) b.week_number = 0;
  // leftover response keys cannot hide a newer week
  if (leftoverResponseKey(aYearKey)) a.season_year = 0;
  if (leftoverResponseKey(aNumKey)) a.week_number = 0;
  if (leftoverResponseKey(bYearKey)) b.season_year = 0;
  if (leftoverResponseKey(bNumKey)) b.week_number = 0;
  // leftover readable stream keys cannot hide a newer week
  if (leftoverReadableStreamKey(aYearKey)) a.season_year = 0;
  ...
  if (leftoverTransformStreamKey(aYearKey)) a.season_year = 0;
  if (leftoverTransformStreamKey(aNumKey)) a.week_number = 0;
  if (leftoverTransformStreamKey(bYearKey)) b.season_year = 0;
  if (leftoverTransformStreamKey(bNumKey)) b.week_number = 0;
  // leftover compression stream keys cannot hide a newer week
  if (leftoverCompressionStreamKey(aYearKey)) a.season_year = 0;
  ...
  // leftover text encoder stream keys cannot hide a newer week
  if (leftoverTextEncoderStreamKey(aYearKey)) a.season_year = 0;
  if (leftoverTextEncoderStreamKey(aNumKey)) a.week_number = 0;
  if (leftoverTextEncoderStreamKey(bYearKey)) b.season_year = 0;
  if (leftoverTextEncoderStreamKey(bNumKey)) b.week_number = 0;
...
export function selectActiveWeek<T extends WeekLike>(
  ...
  for (const week of weeks) {
    ...
    try {
      ...
      // leftover map keys cannot hide a newer week
      if (leftoverMapKey(week.season_year) || leftoverMapKey(week.week_number)) {
        continue;
      }
      // leftover set keys cannot hide a newer week
      if (leftoverSetKey(week.season_year) || leftoverSetKey(week.week_number)) {
        continue;
      }
      // leftover weakmap keys cannot hide a newer week
      if (leftoverWeakMapKey(week.season_year) || leftoverWeakMapKey(week.week_number)) {
        continue;
      }
      // leftover weakset keys cannot hide a newer week
      if (leftoverWeakSetKey(week.season_year) || leftoverWeakSetKey(week.week_number)) {
        continue;
      }
      // leftover promise keys cannot hide a newer week
      if (leftoverPromiseKey(week.season_year) || leftoverPromiseKey(week.week_number)) {
        continue;
      }
      // leftover error keys cannot hide a newer week
      if (leftoverErrorKey(week.season_year) || leftoverErrorKey(week.week_number)) {
        continue;
      }
      // leftover regexp keys cannot hide a newer week
      ...
      // leftover dataview keys cannot hide a newer week
      if (leftoverDataViewKey(week.season_year) || leftoverDataViewKey(week.week_number)) {
        continue;
      }
      // leftover typedarray keys cannot hide a newer week
      if (leftoverTypedArrayKey(week.season_year) || leftoverTypedArrayKey(week.week_number)) {
        continue;
      }
      // leftover blob keys cannot hide a newer week
      if (leftoverBlobKey(week.season_year) || leftoverBlobKey(week.week_number)) {
        continue;
      }
      // leftover file keys cannot hide a newer week
      if (leftoverFileKey(week.season_year) || leftoverFileKey(week.week_number)) {
        continue;
      }
      // leftover url keys cannot hide a newer week
      ...
      // leftover formdata keys cannot hide a newer week
      if (leftoverFormDataKey(week.season_year) || leftoverFormDataKey(week.week_number)) {
        continue;
      }
      // leftover headers keys cannot hide a newer week
      if (leftoverHeadersKey(week.season_year) || leftoverHeadersKey(week.week_number)) {
        continue;
      }
      // leftover request keys cannot hide a newer week
      if (leftoverRequestKey(week.season_year) || leftoverRequestKey(week.week_number)) {
        continue;
      }
      // leftover response keys cannot hide a newer week
      if (leftoverResponseKey(week.season_year) || leftoverResponseKey(week.week_number)) {
        continue;
      }
      // leftover readable stream keys cannot hide a newer week
      ...
      // leftover transform stream keys cannot hide a newer week
      if (leftoverTransformStreamKey(week.season_year) || leftoverTransformStreamKey(week.week_number)) {
        continue;
      }
      // leftover compression stream keys cannot hide a newer week
      ...
      // leftover text encoder stream keys cannot hide a newer week
      if (leftoverTextEncoderStreamKey(week.season_year) || leftoverTextEncoderStreamKey(week.week_number)) {
        continue;
      }
...
export function nextWeekSlot(week: WeekSlot): WeekSlot {
  ...
  // leftover map keys cannot hide a newer week
  try {
    if (leftoverMapKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverMapKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover set keys cannot hide a newer week
  try {
    if (leftoverSetKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverSetKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover weakmap keys cannot hide a newer week
  try {
    if (leftoverWeakMapKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverWeakMapKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover weakset keys cannot hide a newer week
  try {
    if (leftoverWeakSetKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverWeakSetKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover promise keys cannot hide a newer week
  try {
    if (leftoverPromiseKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverPromiseKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover error keys cannot hide a newer week
  try {
    if (leftoverErrorKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverErrorKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover regexp keys cannot hide a newer week
  try {
    if (leftoverRegExpKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
  ...
  // leftover dataview keys cannot hide a newer week
  try {
    if (leftoverDataViewKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverDataViewKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover typedarray keys cannot hide a newer week
  try {
    if (leftoverTypedArrayKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverTypedArrayKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover blob keys cannot hide a newer week
  try {
    if (leftoverBlobKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverBlobKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover file keys cannot hide a newer week
  try {
    if (leftoverFileKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverFileKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover url keys cannot hide a newer week
  try {
    if (leftoverUrlKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
  ...
  // leftover formdata keys cannot hide a newer week
  try {
    if (leftoverFormDataKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverFormDataKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover headers keys cannot hide a newer week
  try {
    if (leftoverHeadersKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverHeadersKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover request keys cannot hide a newer week
  try {
    if (leftoverRequestKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverRequestKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover response keys cannot hide a newer week
  try {
    if (leftoverResponseKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverResponseKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover readable stream keys cannot hide a newer week
  try {
    if (leftoverReadableStreamKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
  ...
  // leftover transform stream keys cannot hide a newer week
  try {
    if (leftoverTransformStreamKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverTransformStreamKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  // leftover compression stream keys cannot hide a newer week
  try {
    if (leftoverCompressionStreamKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
  ...
  // leftover text encoder stream keys cannot hide a newer week
  try {
    if (leftoverTextEncoderStreamKey(week.season_year)) season_year = 0;
  } catch {
    season_year = 0;
  }
  try {
    if (leftoverTextEncoderStreamKey(week.week_number)) week_number = 0;
  } catch {
    week_number = 0;
  }
  if (week_number >= 18) {
...
export function familyWeekChrome(
  ...
  if (week) {
    try {
      ...
      if (leftoverMapKey(weekNumber)) return name;
      if (leftoverSetKey(weekNumber)) return name;
      if (leftoverWeakMapKey(weekNumber)) return name;
      if (leftoverWeakSetKey(weekNumber)) return name;
      ...
      if (leftoverDataViewKey(weekNumber)) return name;
      if (leftoverTypedArrayKey(weekNumber)) return name;
      ...
      if (leftoverFormDataKey(weekNumber)) return name;
      if (leftoverHeadersKey(weekNumber)) return name;
      ...
      if (leftoverTextEncoderStreamKey(weekNumber)) return name;
    } catch {
      // leftover throwing rows cannot hide a newer week
      return name;
    }
...
export function weekSwitcherLabel(w: WeekRef, active: WeekRef | null): string {
  try {
    ...
    // leftover map keys cannot hide a newer week
    if (leftoverMapKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover set keys cannot hide a newer week
    if (leftoverSetKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover weakmap keys cannot hide a newer week
    if (leftoverWeakMapKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover weakset keys cannot hide a newer week
    if (leftoverWeakSetKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover promise keys cannot hide a newer week
    if (leftoverPromiseKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover error keys cannot hide a newer week
    ...
    // leftover dataview keys cannot hide a newer week
    if (leftoverDataViewKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover typedarray keys cannot hide a newer week
    if (leftoverTypedArrayKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover blob keys cannot hide a newer week
    if (leftoverBlobKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover file keys cannot hide a newer week
    ...
    // leftover formdata keys cannot hide a newer week
    if (leftoverFormDataKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover headers keys cannot hide a newer week
    if (leftoverHeadersKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover request keys cannot hide a newer week
    if (leftoverRequestKey(weekNumber)) w = { ...w, week_number: 0 };
    // leftover response keys cannot hide a newer week
    ...
    // leftover text encoder stream keys cannot hide a newer week
    if (leftoverTextEncoderStreamKey(weekNumber)) w = { ...w, week_number: 0 };
  } catch {
    // leftover unconvertible keys cannot hide a newer week
    w = { ...w, week_number: 0 };
  }
