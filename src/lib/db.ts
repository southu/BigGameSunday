...
async function fetchHouseholdWeeks(householdId: string): Promise<Week[]> {
  ...
  return [...weeks]
    ...
    .filter((week) => {
      try {
        return !(week.season_year instanceof File) && !(week.week_number instanceof File);
      } catch {
    ...
    .filter((week) => {
      try {
        return !(week.season_year instanceof URL) && !(week.week_number instanceof URL);
      } catch {
    ...
    .filter((week) => {
      try {
        return (
          !(week.season_year instanceof URLSearchParams) &&
          !(week.week_number instanceof URLSearchParams)
        );
      } catch {
    ...
    .filter((week) => {
      try {
        return !(week.season_year instanceof FormData) && !(week.week_number instanceof FormData);
      } catch {
    ...
    .filter((week) => {
      try {
        return !(week.season_year instanceof Headers) && !(week.week_number instanceof Headers);
      } catch {
    ...
    .filter((week) => {
      try {
        return !(week.season_year instanceof Request) && !(week.week_number instanceof Request);
      } catch {
    ...
    .filter((week) => {
      try {
        return !(week.season_year instanceof Response) && !(week.week_number instanceof Response);
      } catch {
    ...
    .filter((week) => {
      try {
        if (typeof ReadableStream !== "function") return true;
        return (
          !(week.season_year instanceof ReadableStream) &&
          !(week.week_number instanceof ReadableStream)
        );
      } catch {
    ...
    .filter((week) => {
      try {
        if (typeof WritableStream !== "function") return true;
        return (
          !(week.season_year instanceof WritableStream) &&
          !(week.week_number instanceof WritableStream)
        );
      } catch {
    ...
    .filter((week) => {
      try {
        if (typeof TransformStream !== "function") return true;
        return (
          !(week.season_year instanceof TransformStream) &&
          !(week.week_number instanceof TransformStream)
        );
      } catch {
    ...
    .filter((week) => {
      try {
        ...
        return (
          !(week.season_year instanceof CompressionStream) &&
          !(week.week_number instanceof CompressionStream)
        );
      } catch {
    ...
    .filter((week) => {
      try {
        ...
        return (
          !(week.season_year instanceof DecompressionStream) &&
          !(week.week_number instanceof DecompressionStream)
        );
      } catch {
    ...
    .filter((week) => {
      try {
        if (typeof TextEncoderStream !== "function") return true;
        return (
          !(week.season_year instanceof TextEncoderStream) &&
          !(week.week_number instanceof TextEncoderStream)
        );
      } catch {
